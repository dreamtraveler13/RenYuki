...
import { getEpayV2Config, verifyEpaySignature } from '@/lib/epayV2';
...
export async function POST(req: NextRequest) {
  ...
  const { publicKey } = getEpayV2Config();
  if (!verifyEpaySignature(params, publicKey)) return new Response('fail', { status: 400 });

  const order = await getOrderByOutTradeNo(outTradeNo);
  if (!order) return new Response('fail', { status: 404 });

  if (tradeStatus !== 'TRADE_SUCCESS') return new Response('success', { status: 200 });

  const money = (params.money || '').trim();
  if (money && order.amount && money !== order.amount) return new Response('fail', { status: 400 });

  await markOrderPaid({ outTradeNo, tradeNo: params.trade_no || params.api_trade_no, rawNotify: params });
